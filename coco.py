    ...
    if helpers.is_course_url(url):
        with Client(headless=headless) as client:
            ...
            try:
                ...
                course.title = helpers.clean_new_line(course.title)
            ...
            for pfx_s, section in enumerate(course_sections, start=1):
                section.title = helpers.clean_new_line(section.title)
                section.title = helpers.clean_string(section.title)
                for pfx_v, video_url in enumerate(section.videos_url, start=1):
                    if helpers.is_article_url(video_url.url):
                        ...
                        try:
                            ...
                            message = f"[SECTION] {section.title} [VIDEO] {video_url.url}"
                        ...
                        for attempt in range(1, max_retries + 1):
                            try:
                                ...
                                tprint(f"[green]'{pfx_s:02d}. {section.title} / [/green]", end="")
                                tprint(f"[green]{pfx_v:02d}. {video.title}'[/green] ...")
                                client.refresh_cookies()
    ...
    if helpers.is_bootcamp_url(url):
        with Client(headless=False) as client:
            ...
            try:
                ...
                tprint(f"[red]✗[/red] Unable to fetch the name of class or course => {e}")
                cli_logger.error(e)
            confirm_download = typer.confirm("Would you like to download?")
            if not confirm_download:
                raise typer.Exit()
                    ...
                    for video in class_.videos:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                tprint(f"[green]{video.sequence:02d}. {video.title}'[/green] ...")
                                client.refresh_cookies()
                                dir_path = f"{consts.DOWNLOADS_DIR}/"
